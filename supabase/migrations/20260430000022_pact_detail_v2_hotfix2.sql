-- =====================================================================
-- Sprint 4 chunk 4 · Migration 0026 (HOTFIX 2)
-- Corrige columnas inexistentes en sf_get_pact_detail (deposit_movements)
-- =====================================================================
-- En la tabla real (creada en migración 0018) las columnas se llaman:
--   * related_milestone_id   (NO milestone_id)
--   * occurred_at            (NO created_at)
--
-- Solución: usar los nombres reales en el FROM pero mantener los alias
-- del JSON ('milestone_id', 'created_at') para no tocar el cliente Dart.
-- También aprovechamos para exponer related_addendum_id por si en futuras
-- pantallas queremos vincular movimientos con anexos firmados.
-- =====================================================================
  ...
  SELECT jsonb_build_object(
    ...
    'deposit_movements', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
        ...
        -- Columnas REALES: related_milestone_id, related_addendum_id, occurred_at
...
COMMENT ON FUNCTION public.sf_get_pact_detail IS
  'Sprint 4 hotfix 2 · deposit_movements ahora usa related_milestone_id y '
  'occurred_at (columnas reales) mapeadas a milestone_id/created_at en JSON.';
